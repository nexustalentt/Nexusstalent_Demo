...
type StoredAnswer = { selected?: number[]; text?: string } | null;
...
function ExamPage() {
  ...
  if (intro.isLoading) return <Shell>Loading…</Shell>;
...
function LoginCard({
  ...
  const mutation = useMutation({
    ...
    onError: (mutationError: Error) =>
      setError(
        mutationError.message.includes("already submitted")
  ...
  return (
    <Shell>
      ...
      >
        <div>
          <label className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-muted-foreground" htmlFor="exam-username">
            Username
          </label>
          <input
            id="exam-username"
        ...
        <div>
          <label className="mb-1.5 block text-xs font-bold uppercase tracking-wider text-muted-foreground" htmlFor="exam-password">
            Password
          </label>
          <input
            id="exam-password"
